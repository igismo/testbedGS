...
###
# name2var {name}
# Convert an Emulab name back to a tcl variable
###
proc name2var {name} {
    if { [ string equal -length 5 $name "fa-tb"]  } { return $name; }
    if { [ string equal -length 7 $name "tblan-l"] ||
    	 [ string equal -length 8 $name "tbnode-n"] ||
    	 [ string equal -length 8 $name "tblink-l"] } {
	set name "fa-$name"
    } else {
        set idx [ string first "-" $name ];
        while { $idx != -1 } {
	    set name [ string replace $name $idx $idx "("];
	    set idx [ string first "-" $name ];
	    if { $idx != -1 } {
		# The extra dash is a marker that there's another dimension to
		# the array.  Thanks, jshupe.
	        set name [string replace $name $idx $idx ")-"];
	    } else {
	        set name "$name)";
	    }
...
###
# name2var {name}
# Convert an Emulab name back to a tcl variable reference.  Variables with odd
# names need to be bracketed
###
proc name2ref {name} {
    set ref [name2var $name]
    if { [string first "-" $ref] == -1 } {
    	return "\$$ref";
    } else {
    	return "\${$ref}";
    }
...
variable passmode 0
variable splitmode 0
variable include_fedkit 0
variable include_gatewaykit 0
# Maximum number of links to route through a single gateway in split mode.
variable muxmax 3
# This is the Emulab node failure action.  It can be reset by 
# tb-set-default-failure-action
variable default_failureaction "fatal"
variable vtype_list {}
variable master_tb ""
variable testbeds 
array set testbeds { }
# The number of connections between testbeds
variable connections
array set connections { }
# The number of pseudo-nodes representing IP addresses inserted into the
# experiment.  This should be a Node class variable, but this is a consistent
# usage
variable ipaddr_nodes 0
}
...
while {$argv != {}} {
	...
	lpop argv
	set GLOBALS::splitmode 1
    } elseif {$arg == "-t"} {
	lpop argv
	set GLOBALS::splitmode 2
    } elseif {$arg == "-k"} {
	lpop argv
	set GLOBALS::include_fedkit 1
    } elseif {$arg == "-K"} {
	lpop argv
	set GLOBALS::include_gatewaykit 1
    } elseif { $arg == "-x" } {
	lpop argv
	if { [llength $argv] == 0 } {
	    puts stderr "-x reguires a maxmum";
	    usage();
	}
	set GLOBALS::muxmax [lindex $argv 0]
	lpop argv
    } elseif {$arg == "-m"} {
	lpop argv
	if { [llength $argv] == 0 } {
	    puts stderr "-m reguires a testbed name";
	    usage();
	}
	set GLOBALS::master_tb [lindex $argv 0]
	lpop argv
    } else {
	break
    }
...
namespace eval GLOBALS {
    ...
    variable tbroot /usr/testbed
    variable libdir /usr/testbed/lib/ns2ir
    ...
    variable firewall_override "nope"
