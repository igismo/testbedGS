...
func LocateOfficeMaster(myName string, sliceOfMgrs [] tbMessages.TBmgr) (*net.UDPAddr,
	...
	UdpAddress, err = net.ResolveUDPAddr("udp", tbConfig.TBofficeMgr)
	if err != nil {
		fmt.Println(myName,": ERROR locating Office Manager, will retry", err)
		return nil, FullName, false
	}

	FullName = tbMessages.NameId{Name: tbConfig.TBofficeMgrName, OsId: 0,
	...
	theMgr, _ :=  LocateMaster(sliceOfMgrs, tbConfig.TBofficeMgrName)
	if theMgr != nil {
