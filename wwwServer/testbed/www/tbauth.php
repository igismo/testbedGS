...
define("CHECKLOGIN_CVSWEB",     0x010000);      //Obsolete
...
define("CHECKLOGIN_WIKIONLY",       0x200000); //Obsolete
...
define("DOLOGIN_MAXIPATTEMPTS",     50);
...
function GENHASH() {
    ...
    ## GORAN $hash  = mhash (MHASH_MD5, bin2hex($random_bytes) . " " . microtime());
    ## GORAN return bin2hex($hash);
    $hash  = hash ('md5', bin2hex($random_bytes) . " " . microtime());
    return $hash; 
}
...
# Is the user a member of the exec group; used for project approval
function GoranGetUidHash($uid, $getidx) {
   
    $safe_uid = addslashes($uid);
    $query_result = DBQueryFatal(
        "select uid_idx, hashkey from login where uid = '$safe_uid'");

    # there are no rows if the group doesn't exist or the user is not in it
    if (! mysql_num_rows($query_result))
        return 0;
...
function LoginStatus() {
    global $TBAUTHCOOKIE, $TBLOGINCOOKIE, $_COOKIE, $TBAUTHTIMEOUT;
    ...
    ## GORAN TEMP
    #echo "<h3>
    #.... STATUS=$CHECKLOGIN_STATUS  uid_idx=$uid_idx                             
    #</h3>\n";
    #die("");
    ...
    if (array_key_exists($TBLOGINCOOKIE, $_COOKIE)) {
        ...
        $curhash = GoranGetUidHash($uid_idx, 0);
        $uid_idx = GoranGetUidHash($uid_idx, 1);
        $CHECKLOGIN_IDX = $uid_idx; 
    }
...
## GORAN TEMP
#    if (isset($curhash) && ! preg_match("/^[\w]+$/", $curhash)) {
#        $CHECKLOGIN_STATUS = CHECKLOGIN_NOTLOGGEDIN;
#        return $CHECKLOGIN_STATUS;
#    }
#
#    if (isset($hashhash) && ! preg_match("/^[\w]+$/", $hashhash)) {
#        $CHECKLOGIN_STATUS = CHECKLOGIN_NOTLOGGEDIN;
#        return $CHECKLOGIN_STATUS;
#    }
    ...
    #
    $query_result =
    DBQueryFatal("select NOW()>=u.pswd_expires as pswd_expired,l.hashkey,l.timeout, ".
             "       u.status,admin,g.trust,l.adminon,webonly, " .
             "       user_interface,n.type,u.stud, ".
             "       g.pid,u.foreign_admin,u.uid_idx, " .
    ...
    while ($row = mysql_fetch_array($query_result)) {
        $expired = $row['pswd_expired'];
        $hashkey = $row['hashkey'];
        $timeout = $row['timeout'];
        $status  = $row['status'];
        $admin   = $row['admin'];

        if (! strcmp($row['trust'], "project_root") ||
            ! strcmp($row['trust'], "group_root")) {
            $trusted = 1;
        }
        $adminon  = $row['adminon'];
        $webonly  = $row['webonly'];
        $interface= $row['user_interface'];
        ...
        $foreign_admin   = $row['foreign_admin'];
        $uid_idx         = $row['uid_idx'];
        $workbench      += $row['allow_workbench'];
    ...
    #
    if ($frozen || $status == TBDB_USERSTATUS_FROZEN) {
    ...
    if (isset($curhash)) {
        ...
        #
        if (isset($hashhash) && 
            ## GORAN $hashhash == bin2hex(mhash(MHASH_CRC32, $hashkey))) {
            $hashhash == hash('crc32', $hashkey)) {
            #
...
function LOGGEDINORDIE($uid, $modifier = 0, $login_url = NULL) {
    ...
    $redirect_url = null;
    if (!$login_url) {
        $login_url = "$TBBASE/login.php?refer=1";

        if ($uid) {
            # HTTP_REFERER will not work reliably when redirecting so
            # pass in the URI for this page as an argument
            $redirect_url = "$TBBASE/login.php?referrer=".
                                urlencode($_SERVER['REQUEST_URI']);
        }
    }

    $link = "\n<a href=\"$login_url\">Please log in again.</a>\n";
    ...
    switch ($status & CHECKLOGIN_STATUSMASK) {
        case CHECKLOGIN_NOTLOGGEDIN:
            if ($redirect_url) {
                ...
                USERERROR("tbauth.php:You do not appear to be logged in! $link",
    ...
    if ($status & CHECKLOGIN_PSWDEXPIRED)
        USERERROR("Your password has expired. ".
          "<a href=moduserinfo.php>Please change it now!</a>",
          1, HTTP_403_FORBIDDEN);
    ...
    if (NOLOGINS() && ($status & CHECKLOGIN_ISADMIN) != CHECKLOGIN_ISADMIN)
        USERERROR("Sorry. The Web Interface is ".
          "<a href=nologins.php>Temporarily Unavailable!</a>", 1);
...
function ISEXEC() {
    ...
    $safe_uid = addslashes($uid);
    $query_result = DBQueryFatal(
        'select trust != "none" is_member ' .
        '  from group_membership ' .
        " where uid = '$safe_uid' and pid = 'deter-exec'"
    );
    ...
    # return true only when the user's trust is not 'none'
...
function DOLOGIN($token, $password, $adminmode = 0) {
    ...
    if (!TBvalid_uid($token) || !isset($password) || $password == "") {
    ...
    if (isset($_SERVER['REMOTE_ADDR'])) {
        ...
        $ip_result = DBQueryFatal("select * from login_failures ".
                                  "where IP='$IP'");
    ...
    do {
        if ($user) {
            ...
            #
            DBQueryFatal("insert into whitelist values(".
                         "'$uid', INET_ATON( '$IP' ), CURRENT_TIMESTAMP )");
...
function DOLOGIN_MAGIC($uid, $uid_idx, $email = null, $adminon = 0)
{
    ...
    global $TBNAMECOOKIE, $TBLOGINCOOKIE, $TBSECURECOOKIES;
    global $TBMAIL_OPS, $TBMAIL_AUDIT, $TBMAIL_WWW;
    global $TRACSUPPORT, $TRACCOOKIENAME;
    ...
    $hashkey = GENHASH();
    ## GORAN $crc     = bin2hex(mhash(MHASH_CRC32, $hashkey));
    $crc     = hash('crc32', $hashkey);
    ...
    #
    # DETER hack
    # delete cookies for the long isi.deterlab.net domain. This is required to
    # transition to a shorter cookie domain.
    # See https://trac.deterlab.net/ticket/210#comment:10
    $failure = array($TBAUTHCOOKIE, $TBLOGINCOOKIE, $TBNAMECOOKIE);
    foreach ($failure as $fail)
        setcookie($fail, '', 0, "/",
                  '.isi.deterlab.net.', $TBSECURECOOKIES);
    #
    # End DETER
    #
