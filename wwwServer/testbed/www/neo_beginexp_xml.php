...
$project         = null;
$group           = null;
### GORAN  NEO JSON node description file 
$nodesfilelocale    = "";
$thenodesfile       = 0;
$nonodesfile        = 0;
$deletenodesfile    = 0;
...
if (isset($formfields["--loglevel"]) ) {
    $loglevel = $formfields["--loglevel"];
}
...
### GORAN - get JSON file
if (isset($formfields["exp_localnodesfile"]) &&
         $formfields["exp_localnodesfile"] != "") {
     if (!preg_match("/^([-\@\w\.\/]+)$/", $formfields["exp_localnodesfile"])) {
         $errors["Server NS File"] = "Pathname includes illegal characters";
     }
     elseif (! VALIDUSERPATH($formfields["exp_localnodesfile"])) {
         $errors["Server NS File"] =
                 "Must reside in one of: $TBVALIDDIRS";
     }
     $nodesfilelocale = "local";
}
elseif (isset($formfields["exp_nodesfile_contents"]) &&
         $formfields["exp_nodesfile_contents"] != "") {
     #
     # The JSON file is encoded inline. We will write it to a tempfile below
     # once all other checks passed.
     #
     $nodesfilelocale = "inline";
}
...
#### GORAN get the JSON file  local/inline
if ($nodesfilelocale == "local") {
     #
     # No way to tell from here if this file actually exists, since
     # the web server runs as user www. The startexp script checks
     # for the file so the user will get immediate feedback if the filename
     # is bogus.
     #
     $thenodesfile = $formfields["exp_localnodesfile"];
}
elseif ($nodesfilelocale == "inline") {
     #
     # The JSON file is encoded in the URL. Must create a temp file
     # to hold it, and pass through to the backend.
     #
     # Generate a hopefully unique filename that is hard to guess.
     # See backend scripts.
     ...
     $thenodesfile    = "/tmp/$uid-$foo.nodesfile";
     $deletenodesfile = 1;
 
     if (! ($fp = fopen($thenodesfile, "w"))) {
         TBERROR("Could not create temporary file $thenodesfile", 1);
     }
     fwrite($fp, urldecode($formfields["exp_nodesfile_contents"]));
     fclose($fp);
     chmod($thenodesfile, 0666);
}
else {
     $nonodesfile = 1;
}
...
$cmd2 = " ";
$retval = 1000;
if(!$nonsfile && !$nonodesfile) {
    $cmd1 = "webbatchexp $batcharg -E $exp_desc $exp_swappable ".
...
### GORAN ... call NEO function initialize_containers.py
$retval = 2000;
if (!$nonodesfile) { 
    $cmd2 = "NEO_InitContainers ".
            "-p $exp_pid -e $exp_id -f $thenodesfile " .
            " -l $loglevel";

    $retval = SUEXEC($uid, "$exp_pid,$unix_gid" . $extragroups ,
                 "NEO_InitContainers ".
                 "-p $exp_pid -e $exp_id -f $thenodesfile " . 
                 " -l $loglevel", SUEXEC_ACTION_IGNORE);
}
...
#### GORAN TODO .... delete all experiment artifacts
...
if ($nonsfile || $nonodesfile) {
    $message =
         "The following two commands would have been executed: *** " .
         "<p>". $cmd1 . "</p>" .  $cmd2 . " ***  ---  " .
...
elseif ($exp_preload) {
    $message = 
         "The following two commands have been executed: " .
         "<p>" . $cmd1 . "</p>"  . $cmd2 . "<p> " .
         "Since you are only pre-loading the experiment, this will typically
          ...
          http://trac.create.iucc.ac.il/wiki/GettingHelp</p>";
}
elseif ($exp_batched) {
    $message = 
	  ...
	  or days. You will be notified via email when the experiment has
          ...
          http://trac.create.iucc.ac.il/wiki/GettingHelp";
}
else {
    $message = 
         "The following two commands have been executed: *** " .
