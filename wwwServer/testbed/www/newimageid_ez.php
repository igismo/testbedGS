...
if (isset($formfields['simple']) && $formfields['simple']) {
    $view = array(
        'hide_upload' => 1, 'hide_snapshot' => 1, 'hide_partition' => 1, 
        'hide_os' => 1, 'hide_version' => 1, 'hide_filename' => 1, 
        'hide_features' => 1, 'hide_opmode' => 1, 'hide_footnotes' => 1, 
        'hide_types' => 1, 'hide_reboot' => 1, 'hide_required' => 1,
    );
} else {
...
$help_message = 
      "See the
      <a href=$WIKIDOCURL/Tutorial%23CustomOS>
...
function SPITFORM($formfields, $errors)
{
    ...
    $required = '*';
    if (isset($view['hide_required']))
        $required = '';

    if ($isadmin) {
	echo "<center>
               Administrators get to use the
               <a href='newimageid.php'>long form</a>.
              </center>\n";
    }
    ...
    echo "
          <form action='newimageid_ez.php' enctype=\"multipart/form-data\"
    ...
    #
    echo "<tr>
              <td>${required}Image Name (no blanks):</td>
              <td class=left>
                  <input type=text
                         onChange='SetPrefix(idform);'
	...
	echo "<tr>
		  <td>${required}Which DOS Partition[<b>1</b>]:<br>
	...
	echo "<tr>
		 <td>${required}Operating System:<br>
    ...
    # Path to image.
    #
    if (isset($view['hide_filename']))
        spithidden($formfields, 'path');
    else {
        echo "<tr>
    ...
    echo "<tr>
              <td>Node to Obtain Snapshot from$footnote:</td>
              <td class=left>\n";
    if (isset($view["hide_snapshot"])) {
	spithidden($formfields, 'node_id');

        $node_id = $formfields['node_id'];
        echo "<a href=\"shownode.php?node_id=$node_id\" target=\"_blank\">$node_id</a>\n";
    } else {
        echo "
		      <input type=text
    ...
    if (isset($formfields['current_OS'])) {
        $current_OS = $formfields['current_OS'];
        echo "<tr><td>Current OS on node:</td><td>$current_OS</td></tr>\n";
        spithidden($formfields, 'current_OS');
    }
	...
	echo "<tr>
		 <td>${required}Operational Mode[<b>4</b>]:</td>
    ...
    $hide_types = isset($view['hide_types']);
    if (!$hide_types) {
        echo "<tr>
    ...
    while ($row = mysql_fetch_array($types_result)) {
        ...
        if ($hide_types)
            spithidden($formfields, "mtype_$type");
        else {
    ...
    if (isset($view['hide_reboot']))
        spithidden($formfields, 'reboot_waittime');
    else {
        echo "<tr>
    ...
    # make sure we stay in simple mode between submits
    spithidden($formfields, 'simple');
    ...
    if ($formfields['simple']) {
        $url = "newimageid_ez.php";
        if (isset($formfields['node_id']))
            $url .= '?formfields[node_id]=' . $formfields['node_id'];

        echo "<br /><center>Need more options?
            Switch to the <a href=\"$url\">advanced form</a></center>\n";
    } else {
	$url = "newimageid_ez.php?formfields[simple]=1";
        if (isset($formfields['node_id']))
            $url .= '&formfields[node_id]=' . $formfields['node_id'];
 
        echo "<br /><center>Need fewer options?
            Switch to the <a href=\"$url\">SIMPLE form</a></center>\n";
    }
	...
	echo "<blockquote>
		 ...
		 <li> If you don't know what partition you have customized,
		      ...
		      <ul>
		        <li> if you customized one of DETER's Ubuntu 10.04 
			     or greater (see next bullet for Ubuntu 8.04)
			     or CentOS 
			...
			<li> if you customized one of our standard Linux
			     (RHL*) or Fedora images (FC*) or a 
			     DETER Ubuntu 8.04 image then it is
                             partition 2.
			</li>
		    ...
		    <ul>
		      <li> If it is based on a testbed image (one of our
			   Linux, Fedora, FreeBSD, or Windows XP images)
                           ...
                           Select it from the
			   <a href=\"$TBBASE/showosid_list.php\"
...
if (!isset($submit)) {
    ...
    # defaults for creating an image from a running machine
    if (isset($formfields['node_id'])) {
        $ret = valid_node($formfields);

        # inherit from a running machine
        if (isset($ret[1])) {
            inherit($ret[1]);

            # only give an error when it's the simple form
        } else if (isset($formfields['simple']) && $formfields['simple']) {
            USERERROR("Can't load parameters from node: " . $errors['Node']);
	}
    }

    # unable to load it from a node, fall through here
...
if (!isset($formfields["imagename"]) ||
    ...
    $errors["Image Name"] = "Missing Field";
}
...
unset($node_id);
$ret = valid_node($formfields);
if (isset($ret[1])) list($node, $node_id) = valid_node($formfields);
...
if (!isset($confirmed) && 0 != strcmp($confirmationWarning,"")) {
    ...
    echo "<form enctype=\"multipart/form-data\" action='newimageid_ez.php'
            method=post name=idform>";
    #
...
if (! isset($node)) {
    ...
    WRITESUBMENUBUTTON("Edit this Image Descriptor",
		       "editimageid.php?imageid=$fooid");
    WRITESUBMENUBUTTON("Delete this Image Descriptor",
		       "deleteimageid.php?imageid=$fooid");
}
...
#
# Make sure the node is golden
#
function valid_node($formfields) {
    global $errors, $this_user, $TB_NODEACCESS_LOADIMAGE;
...
#
# Inherit fields from OS loaded on node
#
function inherit($node_id) {
    global $formfields, $osid_featurelist;


    $query_result = DBQueryFatal(
        ' select o.osname, o.OS, o.version, o.op_mode, o.osfeatures, o.reboot_waittime, i.loadpart, i.imageid ' .
        '   from os_info o, images i, osidtoimageid oi, nodes n ' .
        '  where o.osid = n.osid and i.imageid = oi.imageid and ' .
        '        oi.osid = n.osid and ' .
        "        n.node_id = '$node_id' " .
        '  group by oi.imageid '
    );
    if (mysql_num_rows($query_result) != 1)
        USERERROR("Can't load parameters from OS on node.  Your node may be in the process of rebooting or imaging.");

    $row = mysql_fetch_array($query_result);

    # only like two ancient images do this, but it's better to fail noisily
    if (!inheritable($row['imageid'])) {
        TBERROR("Uninhertiable OS, imageid " . $row['imageid'], 0);
        USERERROR("The OS you wish to clone is unsupported by this script");
    }

    $formfields['OS'] = $row['OS'];
    $formfields['version'] = $row['version'];
    $formfields['op_mode'] = $row['op_mode'];
    $formfields['current_OS'] = $row['osname'];
    $formfields['reboot_waittime'] = $row['reboot_waittime'];

    # loadpart is 0 if we have a whole disk image, but the form will complain about loadpart being 0
    # on submission.
    if($row['loadpart'] == 0) {
	    $formfields['wholedisk'] = 'Yep';
            $formfields['loadpart'] = 1;
    } else {
            $formfields['loadpart'] = $row['loadpart'];
    }

    # features
    $inherit_feature = array();
    foreach (explode(',', $row['osfeatures']) as $feature)
        $inherit_feature[$feature] = 1;

    reset($osid_featurelist);
    while (list ($feature, $userokay) = each($osid_featurelist))
        if (isset($inherit_feature[$feature]))
            $formfields["os_feature_$feature"] = 'checked';

    # node types
    $query_result = DBQueryFatal(
        'select distinct oi.type ' . 
        '  from osidtoimageid oi, nodes n ' .
        ' where oi.osid = n.osid and ' .
        "       n.node_id = '$node_id' "
    );
    ...
    $query_result = DBQueryFatal(
        'select r.pid, e.gid ' . 
        '  from reserved r ' .
        '  left join experiments e on e.eid = r.eid and e.pid = r.pid ' .
        " where node_id = '$node_id'"
    );
    if (mysql_num_rows($query_result)) {
        $row = mysql_fetch_array($query_result);
        $formfields['pid'] = $row[0];
        if (isset($row[1])) $formfields['gid'] = $row[1];
    }
}

# image is inheritable if it has a single OSID among all node types
function inheritable($imageid) {
    $q = DBQueryFatal(
        'select count(distinct osid) from osidtoimageid ' .
