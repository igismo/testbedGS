...
echo "
      ...
      <br />
      <b>Important group
       <a href='$WIKIDOCURL/Groups%23SECURITY'>
...
echo "<form action='approveuser.php' method='post'>\n";
...
while (list ($uid_idx, $grouplist) = each ($approvelist)) {
    ...
    # Create a searchable list of groups for this user
    $gid_list = array();
    for ($i = 0; $i < count($grouplist); $i++) {
        $gid_list[$i] = $grouplist[$i]->gid();
    }
    ...
    for ($i = 0; $i < count($grouplist); $i++) {
        ...
        # So, if the user is not in the main project group or the main group is not in the
        # list, we are dealing with a subgroup where they can't yet be approved.
        $project->IsMember($user, $ismember);
        if ( $ismember || in_array($pid, $gid_list) ) {
            #
            # Action 
            #
            echo "<td rowspan=2>
            ...
            echo "<td rowspan=2 colspan=2>
                    <b>User is awaiting approval in the main project (the user can not 
be approved at this time).</b>
                    <input type=\"hidden\" name=\"U${uid_idx}\$\$approval-$pid/$gid\" value=\"postpone\">
                    <input type=\"hidden\" name=\"U${uid_idx}\$\$trust-$pid/$gid\" value=\"user\">
                    </td>";
        }
