...
if (isset($view_style) && $view_style == "plab") {
	...
	$view['hide_swapin'] = $view['hide_batch'] = $view['hide_linktest'] =
        $view['quiet'] = $view['plab_ns_message'] = $view['plab_descr'] = 1;
}

if ( ! $isadmin && $swaplock ) {
    # Hide the "Swap In Immediately" form element
    $view['hide_swapin'] = 1;
}

include("beginexp_form.php");
...
if (!isset($beginexp)) {
    ...
    # hide 'batch experiment' from class only users
    if ($this_user->ClassOnly()) {
        $view['hide_batch'] = 1;
        if (!isset($defaults['exp_autoswap_timeout']))
            $defaults["exp_autoswap_timeout"]  = TBGetSiteVar("general/autoswap_threshold_class");
    }
...
# of the results.
#
##### GORAN $uriargs  = "nocookieuid=$uid&nocookieauth=" . $_COOKIE[$TBAUTHCOOKIE];
...
# XXX: As of PHP 5.6, fsockopen validates certificates.  We turn off
# the validation and switch to stream_socket_client so that
# emulab-in-emulab works.  It would be nice to gate the verification,
# but at this moment, we merely restoring the old behavior.
...
##### NEW WAY
##include ("beginexp_xml.php");
##### OLD WAY
#$sock = stream_socket_client($targ, $errno, $errstr,
...
#}
#fputs($sock, "POST $base/beginexp_xml.php?" . $uriargs . " HTTP/1.0\r\n");
...
include ("beginexp_xml.php");
	PAGEHEADER("Begin a Testbed Experiment");
	echo "<br><xmp>";
### GORAN	echo $results["message"];
    echo "STARTING EXPERIMENT CREATION";
