...
class Experiment
{
    ...
    #
    ## GORAN function Experiment($exptidx) {
    function __construct($exptidx) {
	$safe_exptidx = addslashes($exptidx);

	$query_result =
	    DBQueryWarn("select e.* from experiments as e ".
    ...
    static function Lookup($exptidx) {
    ...
    static function LookupByPidEid($pid, $eid) {
    ...
    #
    static function EditExp($experiment, $args, &$errors) {
	...
	$retval = SUEXEC("www", "www", "webeditexp $xmlname",
    ...
    function description()  { return htmlspecialchars($this->field('expt_name')); }
	...
	$uid = $user->uid();
        ...
        # Class projects
        #
        # Special rule that applies only to students:
        #   An experiment in the main project group can only be touched by the
        #   student that created it.
        #
        # Ordinary access checks apply to TA/instructor (>= group_root) and to
        # experiments that are not in the main project group ($gid != $pid)
        #
        if ($project->is_class() && $pid == $gid) {
            # this check only applies to students (< group_root)
            if (!TBMinTrust(TBGrpTrust($uid, $pid, $pid), $TBDB_TRUST_GROUPROOT))
                return $this->creator() == $user->uid();
        }
	...
	$query_result =
	    DBQueryFatal("select e.*, s.archive_idx, ". 
	...
	$exp_gid     = $exprow["gid"];
	$exp_name    = htmlspecialchars($exprow["expt_name"]);
	...
	if (!$short) {
	    ...
	    echo "<tr>
                <td>Project: </td>
                <td class=\"left\">
                  <a href='showproject.php?pid=$pid'>$pid</a></td>
              </tr>\n";

	    echo "<tr>
                <td>Group: </td>
                <td class=\"left\">
                  <a href='showgroup.php?pid=$pid&gid=$exp_gid'>$exp_gid</a>
                </td>
              </tr>\n";
	...
	if (!$short) {
	    ...
	    if (ISADMIN()) {
		echo "<tr>
                    <td><a href='$WIKIDOCURL/Swapping%23swapping'>Swappable:</a></td>
                    <td class=\"left\">$swappable</td>
                  </tr>\n";
	    }
    
	    echo "<tr>
                  <td><a href='$WIKIDOCURL/Swapping%23idleswap'>Idle-Swap:</a></td>
                  <td class=\"left\">$idleswap</td>
              </tr>\n";

	    echo "<tr>
                <td><a href='$WIKIDOCURL/Swapping%23autoswap'>Max. Duration:</a></td>
                <td class=\"left\">$autoswap</td>
              </tr>\n";

	    echo "<tr>
                <td><a href='$WIKIDOCURL/Swapping%23swapstatesave'>Save State:</a></td>
		...
		echo "<tr>
                    <td>Last Error: </td>
                    <td class=\"left\"><a href=\"showlasterror.php?pid=$pid&eid=$eid\">$err_cause</a></td>
                  </tr>\n";
	    }
...
class ExperimentStats
{
    ...
    # Constructor by lookup on unique index.
    #
    ## GORAN function ExperimentStats($exptidx) {
    function __construct($exptidx) {
	$safe_exptidx = addslashes($exptidx);
	...
	$html = "<font size=+2>Experiment <b>".
	    "<a href='showstats.php?showby=expt&exptidx=$idx'>$idx</a> ".
	    "</b></font>\n";
...
class ExperimentResources
{
    ...
    #
    ## GORAN function ExperimentResources($rsrcidx) {
    ...
    static function Lookup($rsrcidx) {
...
function ShowExperimentList($type, $this_user, $object, $tabledefs = null) {
	...
	$query_result =
	    DBQueryFatal("select e.*,count(r.node_id) as nodes, ".
			 ...
			 "     e.eid=r.eid ".
			 "where ($where) and t.guid is null ".
	...
	while ($row = mysql_fetch_array($query_result)) {
	    ...
	    $qresult =
	    DBQueryWarn("select * from risky_experiments where eid='$eid' and pid='$pid' and cxa=0");

 	    if (mysql_num_rows($qresult))   
	     {
	    echo "<tr>$pidrow
                 <td><a href='rshowexp.php?pid=$pid&eid=$eid'>$eid</a></td>
		 <td>$state</td>
