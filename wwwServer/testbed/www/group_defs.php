...
class Group
{
    ...
    # Lookup by gid_idx.
    static function Lookup($gid_idx) {
	global $group_cache;
    ...
    static function LookupByPidGid($pid, $gid) {
    ...
    #
    static function Create($project, $uid, $args, &$errors) {
    ...
    #
    static function EditGroup($group, $uid, $args, &$errors) {
	global $suexec_output, $suexec_output_array;
	...
	elseif ($access_type == $TB_PROJECT_DELUSER) {
	    ...
	    if ($project->is_class()) {
		return ($this->AccessCheck($user, $TB_PROJECT_EDITGROUP));
	    }
    ...
    #
    public static function NewGroup($project, $gid, $leader, $description, $unix_name) {
    ...
    #
    static function Initialize($uid) {
	global $TBOPSPID;
	...
	echo "<tr>
                  <td>GID: </td>
                  <td class=\"left\">
                   <a href='showgroup.php?pid=$pid&gid=$gid'>$gid ($gid_idx)".
	          "</a></td>
              </tr>\n";
	...
	if ($MAILMANSUPPORT) {
	    $mmurl   = "gotommlist.php?pid=$pid&gid=$gid";
	...
	echo "</center>
              <table align=center border=1 cellpadding=1
                     id='userlist' class='sortable' cellspacing=2>\n";
