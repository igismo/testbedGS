...
echo "<b>Show records: ";
if ($showall) {
    echo "<a href='shownodehistory.php?$opts'>allocated only</a>,
          all";
} else {
    echo "allocated only,
          <a href='shownodehistory.php?$opts&showall=1'>all</a>";
}

$opts="node_id=$node_id&count=$count&showall=$showall";
echo "<br><b>Order by: ";
if ($reverse == 0) {
    echo "<a href='shownodehistory.php?$opts&reverse=1'>lastest first</a>,
          earliest first";
} else {
    echo "lastest first,
          <a href='shownodehistory.php?$opts&reverse=0'>earliest first</a>";
}

$opts="node_id=$node_id&showall=$showall&reverse=$reverse";
echo "<br><b>Show number: ";
if ($count != 20) {
    echo "<a href='shownodehistory.php?$opts&count=20'>first 20</a>, ";
} else {
    echo "first 20, ";
}
if ($count != -20) {
    echo "<a href='shownodehistory.php?$opts&count=-20'>last 20</a>, ";
} else {
    echo "last 20, ";
}
if ($count != 0) {
    echo "<a href='shownodehistory.php?$opts&count=0'>all</a>";
} else {
    echo "all";
}
