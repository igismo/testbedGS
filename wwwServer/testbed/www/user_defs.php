...
class User
{
    ...
    #
    static function ValidWebID($token) {
    ...
    # Lookup by uid_idx.
    static function Lookup($uid_idx) {
        global $user_cache;
    ...
    static function LookupByUid($uid) {
    ...
    # Used to prohibit students from starting projects; possibly other things
    static function StudentEmail($email) {
            $safe_email = addslashes($email);

            $query_result =
                DBQueryWarn("select uid from email_aliases ".
            ...
            return User::LookupByUid($idx);
    }
    
    static function LookupByUUID($uuid) {
    ...
    #
    static function ModUserInfo($target_user, $uid, $args, &$errors) {
        ...
        $foo = User::LookupByUid($uid);
        $suexec_uid = (ISADMIN() || $target_user->InstructedBy($foo))
                ? $uid : "www";
        $retval = SUEXEC($suexec_uid, "www", "webmoduserinfo $xmlname",
                 SUEXEC_ACTION_IGNORE);

        # Don't leave sensitive info sitting around in the clear.
        unlink($xmlname);
    ...
    #
    static function NewUserXML($args, &$error) {
        #
        # Generate a temporary file and write in the XML goo.
        #
    ...
    static function NewUser($flags, $args, &$error) {
        global $suexec_output, $suexec_output_array;
        $typearg = "";
        ...
        if (! ($xmlname = User::NewUserXML($args, $error))) {
            return null;
        }

        $retval = SUEXEC("www", "www", "webnewuser $typearg $xmlname",
                 SUEXEC_ACTION_IGNORE);
    ...
    function Show($html = FALSE) {
        ...
        # Email alias.
        #
        $safe_uid =  addslashes($user->uid());
        $query_result =
            DBQueryWarn("select usr_email from email_aliases ".
                        "where uid='$safe_uid'");

        if ($query_result && mysql_num_rows($query_result)) {
            $mfarray = mysql_fetch_array($query_result);
            $usr_email_alias = $mfarray["usr_email"];

            echo "<tr>
                    <td>Aliased to:</td>
                    <td>$usr_email_alias</td>
                  </tr>\n";
        }
        ...
        $isProf = $user->InstructedBy($this_user);
        if (ISADMIN() || $isProf) {
            $freezeflip = ($frozen ? 0 : 1);
            $toggle_url = CreateURL("toggle", $user,
                                    "type", "webfreeze", "value", $freezeflip);
    ...
    #
    # GroupRootEmails - who to alert when necessary
    #
    #   returns a string of the concatenated emails of all group_roots
    #   in all groups of which this user is a member
    # 
    function GroupRootEmails() {

        $this_idx = $this->uid_idx();
        $result = "";

        $query_result = DBQueryWarn("select distinct d.usr_email ".
                                    " from users as d, group_membership as m, group_membership as n ".
                                    " where n.gid_idx=m.gid_idx and m.uid_idx <> $this_idx ".
                                    " and (m.trust='group_root' or m.trust='project_root') ".
                                    " and d.uid_idx=m.uid_idx and n.uid_idx=$this_idx");

        if ($query_result && mysql_num_rows($query_result)) {
            while ( $row = mysql_fetch_array($query_result) ) {
                $thisemail = $row['usr_email'];
                if (!strcmp($result, "")) {
                    $result = $thisemail;
                } else {
                    $result = $thisemail . "," . $result;
                }
            }
        }
    ...
    #
    # CourseAcct, determines if $this is a course account.
    #
    #   returns the pid of the source if so;
    #   returns null otherwise.
    # 
    function CourseAcct() {

        $this_idx = $this->uid_idx();
        $basename = substr($this->uid(), 0, -2);

        $query_result =
            DBQuery("select a.pid from project_attributes as a ".
                    "where a.attrkey='class_idbase' and ".
                    "     a.attrvalue='$basename'");
    ...
    #
    # InstructedBy, determines if $this is taking a class from $user
    #
    #   returns 1 if so.
    #   returns 0 if not allowed.
    # 
    function InstructedBy($user) {
        ...
        $query_result =
            DBQueryFatal("select g.trust from group_membership as g ".
                         "join group_membership as authed on ".
                         "     g.pid_idx=authed.pid_idx and ".
                         "     g.gid_idx=authed.gid_idx and ".
                         "     g.uid_idx='$this_idx' ".
                         "join projects as p on ".
                         "     g.pid_idx=p.pid_idx and p.class<>0 ".
                         "where authed.uid_idx='$auth_idx' and ".
                         "      (authed.trust='group_root' or ".
                         "       authed.trust='project_root')");
    ...
    #
    # IsStudent returns true if the user is a student in any class
    #
    function IsStudent() {
        $this_idx = $this->uid_idx();	
        ...
        $sql =
            'select uid ' .
            '  from group_membership g join projects p '.
	    ' on p.pid = g.pid where p.class=1 and g.pid=g.gid and ' .
            '       g.trust in ("local_root", "user") ' . 
	    ' and uid="' . $this->uid() . '"';
    ...
    #
    # IsTeacherOrTA returns true if the user is teacher or TA in any class
    #
   function IsTeacherOrTa() {
        ...
        $sql =
            'select uid ' .
            '  from group_membership g join projects p '.
	    ' on p.pid = g.pid where p.class=1 and ' .
    ...
    #
    # IsGroupOrProjectRoot returns true if the user is group root in a given pid
    #
    function IsGroupOrProjectRoot($pid) {
        $this_idx = $this->uid_idx();	

        $sql =
            'select uid ' .
            '  from group_membership g join projects p '.
	    ' on p.pid = g.pid where p.pid="' . $pid . '" and ' .
            '       g.trust in ("group_root", "project_root") ' . 
	    ' and uid="' . $this->uid() . '"';
    ...
    #
    # PIof, returns a list of project PIDs of which this user is a PI. Takes
    # and optional class_only flag, which when true limits the result list to
    # class projects only.
    #
    #   returns a (possibly empty) list of PIDs
    #
    function PIof($class_only = 0) {
        $this_idx = $this->uid_idx();

        # $query_result = DBQueryFatal(
        $sql =
            'select p.pid ' .
            '  from group_membership g, projects p '.
            ' where g.pid_idx = p.pid_idx and ' .
    ...
    function SetWebFreeze($freeze) {
        ...
        # clear the various other webfreeze fields
        if ( $freeze == 0 ) {
            DBQueryFatal("update users set ".
    ...
    function ProjectAccessList($access_type) {
        ...
        elseif ($access_type == $TB_PROJECT_MAKEOSID ||
            ...
            if (ISADMIN()) {
                ...
                $trust_clause = "(trust='$trust_project' or trust='$trust_group' or ".
                                " trust='$trust_local')";
    ...
    #
    # Return a true value if the user is only a member of class projects. This
    # method can be used to customize the user interface for class users.
    #
    function ClassOnly() {
        $uid_idx = $this->uid_idx();

        $query_result =
            DBQueryFatal('select count(*) = 0 count ' .
                         '  from group_membership m, projects p ' .
                         ' where m.pid_idx = p.pid_idx and m.pid_idx = m.gid_idx and ' .
                         '       m.trust != "none" and !p.class and '.
                         "       m.uid_idx = '$uid_idx'");
