...
# Otherwise, default to foo.php if not in the list.
#
$url_mapping			    = array();
...
function ValidateArgument($name, $arg)
{
    ...
    case PAGEARG_DATE:
    	$date=explode("-", $arg);
  	if (count($date) < 3)
     	   PAGEARGERROR("'$arg' is not a valid date in yyyy-mm-dd format");
        if (!checkdate($date[1], $date[2], $date[0]))
	   PAGEARGERROR("'$arg' is not a valid date");
        break;
