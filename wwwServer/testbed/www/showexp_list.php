...
if ($showtype != "active") {
    echo "<a class='static' href='showexp_list.php?showtype=active&sortby=$sortby&thumb=$thumb'>Active</a>, ";
} else {
    echo "Active, ";
}

if ($showtype != "batch") {
    echo "<a class='static' href='showexp_list.php?showtype=batch&sortby=$sortby&thumb=$thumb'>Batch</a>, ";
} else {
    echo "Batch, ";
}
...
if ($isadmin) {
    if ($showtype != "idle") {
	echo "<a class='static' href='showexp_list.php?showtype=idle&sortby=$sortby&thumb=$thumb'>Idle</a>, ";
    } else {
	echo "Idle, ";
    }
}
...
if ($showtype != "all") {
    echo "<a class='static' href='showexp_list.php?showtype=all&sortby=$sortby&thumb=$thumb'>All</a>";
} else {
    echo "All";
}
...
if (!$idle) {
    ...
    if ($thumb != 0) {
	echo "<a class='static' href='showexp_list.php?showtype=$showtype&sortby=$sortby&thumb=0'>";
	echo "List";
        echo "</a>";
    } else {
        echo "List";
    ...
    if ($thumb != 1) {
	echo "<a class='static' href='showexp_list.php?showtype=$showtype&sortby=$sortby&thumb=1'>";
    ...
    if ($thumb != 2) {
	echo "<a class='static' href='showexp_list.php?showtype=$showtype&sortby=$sortby&thumb=2'>";
...
if ($thumb && !$idle) {
	...
	echo "<a class='static' href='showexp_list.php?showtype=$showtype&sortby=pid&thumb=$thumb'>";
	echo "Project";
        echo "</a>, ";
    } else {
        echo "Project, ";
    ...
    if ($sortby != "eid") {
	echo "<a class='static' href='showexp_list.php?showtype=$showtype&sortby=eid&thumb=$thumb'>";
	echo "Experiment ID";
        echo "</a>, ";
    } else {
        echo "Experiment ID, ";
    ...
    if ($sortby != "pcs") {
	echo "<a class='static' href='showexp_list.php?showtype=$showtype&sortby=pcs&thumb=$thumb'>";
	echo "Node Usage";
        echo "</a>, ";
    } else {
        echo "Node Usage, ";
    ...
    if ($sortby != "name") {
	echo "<a class='static' href='showexp_list.php?showtype=$showtype&sortby=name&thumb=$thumb'>";
    ...
    if ($sortby != "uid") {
	echo "<a class='static' href='showexp_list.php?showtype=$showtype&sortby=uid&thumb=$thumb'>";
	echo "Creator";
        echo "</a>";
    } else {
        echo "Creator";
...
date_format(expt_swapped,'%m/%d') as dshort, 
unix_timestamp(expt_swapped) as unix_swapped,
...
date_format(expt_swapped,'%m/%d') as dshort, count(r.node_id) as ncount, 
unix_timestamp(expt_swapped) as unix_swapped,
(unix_timestamp(now()) - unix_timestamp(max(greatest(
last_tty_act,last_net_act,last_cpu_act,last_ext_act)))) as idlesec,
(max(last_report) is not null) as canbeidle, s.rsrcidx,
...
left join projects p on e.pid = p.pid
left join experiment_stats as s on e.idx=s.exptidx
left join reserved as r on e.pid=r.pid and e.eid=r.eid 
left join nodes as n on r.node_id=n.node_id 
left join node_activity as na on r.node_id=na.node_id
...
if ($thumb && !$idle) {
	...
	if (! ($head_user = User::Lookup($huid))) {
        ...
        if (! ($swapper_user = User::Lookup($sidx))) {
            TBERROR("Could not lookup object for swapper ($sidx)", 1);
        }
	...
	$head_affil_text = $head_affil ? "&nbsp;($head_affil)" : "";
        ...
        $swapper = $swapper_user->Uid();
        $swapper_url = CreateURL("showuser", $swapper_user);
        $swapper_affil = $swapper_user->affil_abbrev();
        $swapper_affil_text = $swapper_affil ? "&nbsp;($swapper_affil)" : "";
	...
	if ($thumb == 2) {
		...
		echo "<td align=center>".
		     ...
		     "<br>\n".
		     "<b>".
		     "<a href='showproject.php?pid=$pid' title='$pdsc'>$pid</a>/<br />".
		     "<a href='showexp.php?pid=$pid&eid=$eid' title='$desc'>$eid</a>".
		     "</b>".
		     "</td>";
		...
		if ($state=="active") {
		    ...
		    $lastlogin .= "$date swapin";
                    ...
                    # display head and swapper log in time
                    $head_last = TBUsersLastLogin($huid);
                    $swapper_last = TBUsersLastLogin($swapper);
                    if($head_last && $swapper != $huid) {
                            $head_last_date = date("m/d", strtotime($head_last["date"]));
                            $lastlogin .= "<br>$head_last_date head login";
                    }
                    if ($swapper_last) {
                        $swapper_last_unix = strtotime($swapper_last["date"]);

                        $swapper_last_date = date("m/d", $swapper_last_unix);
                        $lastlogin .= "<br>";
                        if ($swapper_last_unix < $unix_swapped)
                            $lastlogin .= '<font color="red"><b>';
                        $lastlogin .= "$swapper_last_date&nbsp;swapper&nbsp;login";
                        if ($swapper_last_unix < $unix_swapped)
                            $lastlogin .= '</b></font>';
                    }
		}
	    ...
	    echo "<font size=-2><b>Swapped by:</b> ".
		 "<a href='$swapper_url'>$swapper</a>$swapper_affil_text".
		 "</font><br />\n";
	...
	echo "<th width=60%>
                  <a class='static' ".
                     "href='showexp_list.php?showtype=$showtype".
                     "&sortby=name$ni'>Description</a></th>\n";
    }
    echo "  <th width=4%>
               <a class='static' ".
                  "href='showexp_list.php?showtype=$showtype&sortby=uid$ni'>
                  Head UID</a></th>
            </tr>\n";
	...
	if (! ($head_user = User::Lookup($huid))) {
        ...
        if (! ($swapper_user = User::Lookup($swapper_idx))) {
            TBERROR("Could not lookup object for swapper ($swapper_idx)", 1);
        }
        $swapper = $swapper_user->Uid();
