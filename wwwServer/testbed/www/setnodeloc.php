		  ...
		  $delete = 0)
{
    ...
    echo "<center>
          ...
          <form action=setnodeloc.php method=get>
          <table class=nogrid
                 align=center border=0 cellpadding=6 cellspacing=0>
          <tr>
             <td>Contact Name:</td>
    ...
    echo "</table>
	  ...
	  <input type=hidden name=floor value=$floor>
          <input type=image
                 src=\"floormap_aux.php?prefix=$uniqueid\">
          </form>
          </center>\n";
}
...
elseif (isset($dodelete) && $dodelete != "") {
	...
	DBQueryFatal("update new_nodes set " .
		     ...
		     "where new_node_id='$node_id'");
	header("Location: newnode_edit.php?id=$node_id");
    }
    else {
	DBQueryFatal("delete from location_info where node_id='$node_id'");
	header("Location: shownode.php?node_id=$node_id");
    }
...
if (!isset($building) || $building == "" || !isset($floor) || $floor == "") {
    ...
    if (isset($old_building)) {
	echo "<a href='setnodeloc.php?node_id=$node_id&delete=1".
	         "&isnewid=$isnewid'>
	...
	echo "<td>
                <a href='setnodeloc.php?node_id=$node_id".
...
# 
header("Location: floormap.php?building=$building&floor=$floor");
