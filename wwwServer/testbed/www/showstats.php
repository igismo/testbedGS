...
echo "<b>Show: <a class='static' href='showexpstats.php'>
                  Experiment Stats</a>";
if ($isadmin) {
    echo "<a class='static' href='showsumstats.php'>, Summary Stats</a>\n";
}
echo "</b><br>\n";
...
if ($showby != "user") {
    echo "<a class='static' href='showstats.php?showby=user'>User</a>, ";
}
else {
    echo "User, ";
}
...
if ($showby != "all") {
    echo "<a class='static' href='showstats.php?showby=all'>";
    if ($isadmin)
	echo "All</a>.";
    else
	echo "Project</a>.";
...
elseif ($showby == "expt") {
    ...
    echo "<font size=+2>";
    echo "(<a href='showexpstats.php?record=$exptidx'>$exptidx</a>) ";
    echo "</font>";
    echo "</center>\n";
    echo "<br>\n";
}
...
while ($row = mysql_fetch_assoc($query_result)) {
	...
	if ($archive_idx && $archive_tag &&
	    ...
	    echo "  <td align=center>
                       <a href=beginexp_html.php?copyid=$exptidx:$archive_tag>
	...
	echo "<td>$uid</td>
              <td>$pid</td>
              <td>$eid</td>
              <td><a href=showexpstats.php?record=$exptidx>$exptidx</a></td>";
    }
    echo "  <td>$start</td>
            <td>$end</td>\n";
