...
$queries["project"] = "select r.pid as project,count(r.node_id) as pcs," . 
    ...
    " where nt.class='pc'" . 
    "  and r.pid != 'emulab-ops'" . 
    " group by r.pid order by pcs desc";

$queries["experiment"] = "select concat_ws(' / ',r.pid,r.eid) as experiment," .
    "  count(r.node_id) as pcs," .
    "  coalesce(min(unix_timestamp(now()) - unix_timestamp(greatest(na.last_tty_act,na.last_net_act,na.last_cpu_act,na.last_ext_act))), 0) as idle," . 
#"  (unix_timestamp(now()) - unix_timestamp(na.last_report)) as stale," . 
    "  coalesce(max(unix_timestamp(now()) - unix_timestamp(e.expt_swapped)), 0) as duration" . 
    " from reserved as r" . 
    " left join nodes as n on r.node_id=n.node_id" . 
    " left join node_types as nt on n.type=nt.type" . 
