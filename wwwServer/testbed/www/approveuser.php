...
while (list ($header, $value) = each ($_POST)) {
	...
	TBERROR("Parse error finding user in approveuser.php", 1);
    }
    if (!$project || strcmp($project, "") == 0) {
	TBERROR("Parse error finding project in approveuser.php", 1);
    }
    if (!$group || strcmp($group, "") == 0) {
	TBERROR("Parse error finding group in approveuser.php", 1);
    }
    if (!$approval || strcmp($approval, "") == 0) {
	TBERROR("Parse error finding approval in approveuser.php", 1);
    }
	...
	TBERROR("Parse error finding trust in approveuser.php", 1);
    }
	...
	TBERROR("Invalid trust $newtrust for user $user approveuser.php.", 1);
    }
    ...
    if (strcmp($approval, "postpone") &&
        ...
        TBERROR("Invalid approval value $approval in approveuser.php.", 1);
    }
	...
	echo "<p>
                  Membership status for user $target_uid in $project/$group was
    ...
    if (! $target_group->AccessCheck($this_user, $TB_PROJECT_ADDUSER)) {
            USERERROR("You are not allowed to approve users in $project/$group!", 1);
    }
