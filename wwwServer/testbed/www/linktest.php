...
function start_linktest($level) {
	...
	echo "Linktest reported errors! Stopped at " .
	    date("D M d G:i:s T") . "\n";
	echo "(If you are mixing pc and bpc nodes, some links may span the " .
	    "physical connection between the USC and UCB clusters, " .
	    "which can insert unpredicatble delays.  If this is an issue ".
	    "restrict your nodes to all pc or all bpc nodes if possible.)\n";
    }
    elseif ($retval < 0) {
	echo $suexec_output;
    }
...
if (isset($_REQUEST["kill"])) {
    stop_linktest();
    header("Location: showexp.php?pid=$pid&eid=$eid");
    return;
}
...
echo "<font size=+2>Experiment <b>".
	...
	"<a href='showexp.php?pid=$pid&eid=$eid'>$eid</a></b></font>\n";
...
echo "<br>\n";
echo "<form action=linktest.php method=post name=myform id=myform>";
