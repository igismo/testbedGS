...
class Image
{
    ...
    static function LookupByName($project, $name) {
	$pid       = $project->pid();
    ...
    #
    static function NewImageId($ez, $imagename, $args, &$errors) {
	...
	$retval = SUEXEC("www", "www", "$script $xmlname",
    ...
    #
    static function EditImageid($image, $args, &$errors) {
	global $suexec_output, $suexec_output_array;
	...
	$retval = SUEXEC("www", "www", "webeditimageid $xmlname",
			 SUEXEC_ACTION_IGNORE);
	...
	$imageid = $this->imageid();
	$query_result = DBQueryWarn("select distinct FROM_UNIXTIME(ih.stamp) ".
		"as last_setup ".
		"from image_history as ih where ih.imageid=$imageid ".
		"order by ih.stamp desc limit 1 ");
	if (mysql_num_rows($query_result) == 0) {
	    return date("Y-m-d H:i:s", 0);
	}
	$row = mysql_fetch_array($query_result);
	return $row["last_setup"];
    }
	...
	echo "<tr>
                  <td>Group: </td>
                  <td class=\"left\">
                    <a href='showgroup.php?pid=$pid&gid=$gid'>$gid</a></td>
              </tr>\n";
