...
function SPITFORM($formfields, $errors)
{
    ...
    #
    $query_result =&
	$target_user->TableLookUp("user_pubkeys", "*", "external=1");
    ...
    echo "<blockquote><blockquote><blockquote>
          <ol>
            <li> Please consult our
                 <a href = '$WIKIDOCURL/SecReqs%23SSH'>
                 security policies</a> for information
                 regarding ssh public keys.
            <li> You should not hand edit your your authorized_keys file on
                 Emulab (/etc/ssh/external_keys/$target_uid/authorized_keys) since modifications via this
                 page will overwrite the file.
            <li> Typically, the file you want to upload is your
                 id_rsa.pub, contained in your .ssh directory.  
            <li> If you use Macintosh OS X, you can navigate to your .ssh directory by following the 
                 instructions <a href='https://trac.deterlab.net/wiki/DETERSSH#OSX'>here</a>.
            <li> As a security precaution, you must supply your password
                 when adding new ssh public keys. 
            <li> Protocol 2 keys are <b>required</b>.
          </ol>
          </blockquote></blockquote></blockquote>\n";

    echo "<font color=red>NOTE:</font> We use the
          <a href=\"http://www.openssh.org\">OpenSSH</a> key format, which has
	  a slightly different protocol 2 public key format than some of the
	  commercial vendors such as <a href=\"http://www.ssh.com\">SSH 
	  Communications</a>. If you use one of these commercial vendors,
	  then please upload the public key file and we will convert it for
	  you. <i>Please do not paste it in.</i>\n";
}
...
function NewPubKey($uid, $args, &$errors) {
    ...
    $suexec_uid = ISADMIN() ? $uid : "www";
    $retval = SUEXEC($suexec_uid, "www", "webaddpubkey -X $xmlname",
