...
foreach ($_GET as $key => $value) {
    if (preg_match("/iface(name|mac|driver|role)(\d+)/",$key,$matches)) {
        $vartype = $matches[1];
    	$ifacenum = $matches[2];
    	if ($vartype == "name") {
	...
	} else if ($vartype == "role") {
            $interfaces[$ifacenum]["role"] = $value;
	} else {
...
#
# Lock the new_nodes table to ensure unique names
#
DBQueryFatal("LOCK TABLES new_nodes WRITE, nodes WRITE, interfaces WRITE");

if (!isset($node_id)) {
    $name_info = find_free_id("pc");  # This either hits xmlrpc with EinE or selects from new_nodes and nodes
...
#
# Attempt to come up with an IP address for it - unless one was
# provided by the client.
#
if (isset($use_temp_IP)) {
    $IP = $tmpIP;
} else {
    $IP = guess_IP($node_prefix,$node_num); # Reads interfaces and new_nodes
}
...
#
# Unlock the new_nodes and interfaces tables
#
...
foreach ($interfaces as $interface) {
	...
	DBQueryFatal("insert into new_interfaces set " .
	    "new_node_id=$new_node_id, card=$card, mac='$mac', role='$role',  " .
	    "interface_type='$type'");
}
...
function find_free_id($prefix) {
	...
	$results = XMLRPC("www", $TBADMINGROUP,
