...
class Node
{
    ...
    # Lookup by node_id
    static function Lookup($node_id) {
	global $node_cache;
	...
	$html = "<font size=+2>Node <b>".
	    "<a href=shownode.php?node_id=$node_id><b>$node_id</a>";
	    "</b></font>\n";
	...
	if ($isvirtnode) {
		...
		echo "<tr>
                      <td>Phys ID:</td>
                      <td class=left>
	   	          <a href='shownode.php?node_id=$phys_nodeid'>
                             $phys_nodeid</a></td>
                  </tr>\n";
	    }
	}
	...
	echo "<tr>
              <td>Node Type:</td>
              <td class=left>
  	          <a href='shownodetype.php?node_type=$type'>$type</td>
          </tr>\n";
	...
	if (!$short && !$noperm) {
		...
		echo "<tr>
                      <td>Reserved Pid:</td>
                      <td class=left>
                          <a href='showproject.php?pid=$reserved_pid'>
                               $reserved_pid</a></td>
                  </tr>\n";
	    }
	    if ($uuid) {
	...
	if (!$short) {
	    ...
	    echo "<tr><th>Interface: MAC</th><th>Model; protocols</th>\n";
	...
	if (! ($user = User::Lookup($contact_uid))) {
            # This is not an error since the field is set to "www" when
...
function ShowNodeHistory($node = null,
    ...
    if ($fp = popen("$TBSUEXEC_PATH www www ".
...
function ShowFreeNodes($user, $group)
{
	...
	if (($row[0] == TBDB_NODESTATE_PXEWAIT) ||
	...
	$output .= "<td class=usagefreenodes align=right>
                     <a target=_parent href=shownodetype.php?node_type=$key>
                        $key</a></td>
