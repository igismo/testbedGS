...
if (isset($research)) {
    ...
    foreach ($mac_list as $mac => $switchport) {
	    ...
	    echo "<h4>$switchport[node_id]:$switchport[card] found on $switchport[switch], updated.";
	} else {
	    echo "<h4>$switchport[node_id]:$switchport[card] not found, not updating</h4>\n";
	}
    }
}
...
<h3><a href="newnodes_list.php">Refresh this page</a></h3>

<form action="newnodes_list.php" method="get" name="nodeform">
...
while ($row = mysql_fetch_array($nodes_result)) {
	...
	echo "          <td><a href=setnodeloc.php?node_id=$id&isnewid=1>";
	if ($building) {
	...
	echo "</a></td>\n";
	echo "<td><a href=\"newnode_edit.php?id=$id\">Edit</a></td>\n";
	echo "	</tr>\n";
}
