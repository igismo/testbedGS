...
$target_isstudent = $target_user->CourseAcct();
$remove = "remove";
if (isset($target_isstudent) && $target_isstudent) {
    $remove = "recycle";
    if (!(isset($target_project) && $target_project)) {
	$target_project = Project::LookupByPid($target_isstudent);
    }
}
...
if (count($experimentlist) && !isset($target_isstudent)) {
    echo "<center><h3>
...
if (isset($target_project)) {
    STARTBUSY("User '$target_uid' is being $remove"."d from '$target_pid'!");
}
else {
    STARTBUSY("User '$target_uid' is being $remove"."d!");
}
...
if (isset($target_isstudent) && $target_isstudent) {
    ...
    putenv("USER=$uid");
    shell_exec
      ("/usr/testbed/sbin/instacct $target_isstudent recycle_one $target_uid");
    putenv("USER=$webuser");
} else {
