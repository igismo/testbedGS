...
var Virt_lans struct {
	Row struct {
		...
		Delay          float32 `xml:"delay" 		json:"delay"`
...
var virtualTablesRows =  map[string] [] map[string]interface{} {
	//var virtualTablesRows = map[string]map[string]interface{} {
	// map[tablename].TABLEROW (map[string]interface{}) ....
...
func processXmlElement(decoder *xml.Decoder, token xml.Token,
	depth *int, myVirtualTablesRows map[string][]map[string]interface{} ) error {
	...
	case xml.StartElement:
		*depth++ // just for testing
		...
		fmt.Println("processXmlElement: decode  .........." + inTable)
		...
		fmt.Println("processXmlElement process:",string(jsonString))
/* WAS   TODO
		...
		fmt.Println("processXmlElement: APPEND to ",inTable,"  ourRowMap:", ourRowMap)
		myVirtualTablesRows[inTable] = append(myVirtualTablesRows[inTable], ourRowMap)
*/
		//rowMap := make(map[string]string)

		rowMap := myVirtualTablesRows[inTable]

		// table := myVirtualTablesRows[inTable] // [] map[string]interface{}
		newRow   := make(map[string]interface{})
		err := json.Unmarshal(jsonString, &newRow)

		myVirtualTablesRows[inTable] = append(myVirtualTablesRows[inTable], newRow)

		if err != nil { return errors.New( "Bad unmarshall of rowMap")}
		fmt.Println("processXmlElement: APPEND to ",inTable,"  ourRowMap:", rowMap)
		//myVirtualTablesRows[inTable] = append(myVirtualTablesRows[inTable], rowMap)
		//=================
		*depth--
		//}
...
//=============================================================================
func usage() {
fmt.Println("Usage: xmlconvert [-x <xmlfile> [-n] [-p]] [-d] pid eid\n")
// ndsp
// s .. simparse
// d .. debug
// p .. fromparser\
// x .. fromxml   $xmlfile = $options{"x"};
// n .. impotant
}
//TODO
func main1() {
...
//=============================================================================
// system("$TB/libexec/xmlconvert -p -x xmlFile $pid $eid");
//=============================================================================
func expXMLconvert(db *sql.DB, exp *tbExpUtils.ExperimentType, inFile string) error {
	fmt.Println("expXMLconvert: START  nsfile:", inFile)
	simparse := false // -s option
	debug    := true  // -d option
	impotent := false // -n option
	isnsfile := false // TODO for now, later check which one it is NS or XML

	flag.Parse()
	fmt.Println("Open file " + inFile)
	xmlFile, err := os.Open(inFile)
	if err != nil {
		fmt.Println("expXMLconvert: Error opening file:", err)
		return err
	}
	defer xmlFile.Close()
	/* The following could be used to detect first two lines in an xml file ...
	...
	*/
	...
	//return
	//}
	myVirtualTablesRows := make(map[string][]map[string]interface{})
	...
	for {
		...
		xmlErr := processXmlElement(decoder, token, &depth, myVirtualTablesRows)
		if err != nil {
			fmt.Println("ERROR:" + xmlErr.Error())
			return err
		}
	...
	// Once here everything should be contained in the myVirtualTablesRows

	//# Create a virt topology object. Do not load the current tables
	fmt.Println("expXMLconvert: Create a virt topology object")
	virtExp := *VirtExpCreateNew(db, exp) // & exp.VirtTables
	...
	if simparse == false {
		...
		fmt.Println("expXMLconvert: check number of exp rows=", myVirtualTablesRows["experiments"])
		// TODO is the bellow required ??
		//if len(myVirtualTablesRows["experiments"]) != 1 {
		//	fmt.Println("Must be exactly one experiments table row!")
		//	return errors.New("Bad unmarshall of rowMap")
		//}
	}

	// TODO CURRENTLY HERE
	// First update the experiments table part of the virt experiment.
	//table := exp.VirtTables["experiments"]
	// my %table = %{@{$virtual_tables{"experiments"}->{"rows"}}[0]};

	// Note that at this point the virtual_tables{"experiments"} is loaded
	// from the rows in the old experiment tables (all fields, like wa_delay_...)
	// for all rows that are enabled in ExperimentFields (the changable stuff)
	// we run TCL function with the value indicated
	// the rows are created in VirtTableCreate using     $self->{'EXPERIMENT'} = $experiment;
	//  virtual_tables = map[string] map[string]string
	// "experiments":   {"rows": "",   "tag":"settings", "row":""},
	fmt.Println("expXMLconvert: Check if simparse")
	if simparse == false {
		// Once here everything should be contained in the myVirtualTablesRows
		// and we hav exp.VirtTables[tablenames].TABLEROW initialized
		var arrayOfRows= myVirtualTablesRows["experiments"] // array of map[string]interface{}
		//var expMap map[string]interface{}
		//expMap = table[0]

		for index, element := range arrayOfRows {
			fmt.Println("expXMLconvert: experiments index=", index, "  element=", element)
			for  key, val := range element {
				_, present := ExperimentFields[key] //in expVirtExp.go
				if present == false {
					continue
				}
				//val := expMap[key]
				if key != "" {
					// TODO function calls ....
					vtable := virtExp["experiments"]
					fmt.Println("expXMLconvert: CALL key=",key," val=",val,"vtable=",vtable.TABLEROW)
					// 	example: KEY=use_ipassign  VAL=0
					// CallFunction(vtable.TABLEROW, key, val // TODO
				}
			}
		}
	}
	//# Now all the other tables.
	fmt.Println("expXMLconvert: Handle all other tables")
	for table := range  virtual_tables {
		//# Don't want to muck with this table. Done above.
		if table == "experiments" {
			continue // already done above
		}
		fmt.Println("expXMLconvert: handle rows for table=", table)
		var arrayOfRows= myVirtualTablesRows["experiments"] // array of map[string]interface{}

		//# The external_sourcefiles table is treated special. Might
		//# become a real table later, once we decide if its useful.

		for rowref := range arrayOfRows {
			fmt.Println("expXMLconvert:   table=", table," rowref=",rowref)
			var rowhash= make(map[string]interface{})
			rowhash = arrayOfRows[rowref]
			if len(rowhash) == 0 {
				continue
			}

			if table == "external_sourcefiles" {
				fmt.Println("expXMLconvert: handle rows for external_sourcefiles table len=", len(arrayOfRows))
				val, present := rowhash["pathname"]
				if (present == true && val != "") {
					pathname := val
					fmt.Println("xpXMLconvert: Add input file ", pathname)

					if tbExpUtils.ExpAddInputFile(db, exp, pathname.(string), isnsfile) == false {
						fmt.Println("Failed to add input file $pathname!")
						return errors.New("Failed to add input file $pathname!")
					}
				}
				continue
			} else { // all other tables
				fmt.Println("expXMLconvert: table=",table," rowref=", rowref, " rowhash=",rowhash)
				for  key, val := range rowhash {

					if exp.VirtTables == nil {
						fmt.Println("expXMLconvert: exp.VirtTables == nil, Could not add" )
						return errors.New( "expXMLconvert: exp.VirtTables == nil,Could not add" )
					}
					fmt.Println("expXMLconvert: table=",table," key=",key,"  val=",val)
					exp.VirtTables[table].TABLEROW[key] = val
				}
			}
		}
	}

	if debug == true { VirtExpDump(exp)}

	//# Now store it.
	var flags = 0
	if (debug)    {flags |= STORE_FLAGS_DEBUG}
	if (impotent) {flags |= STORE_FLAGS_IMPOTENT}
	if (simparse) {flags |= STORE_FLAGS_SIMPARSE}
	if VirtExpStore(db, exp, flags) == false {
		fmt.Println("Could not store new experiment topology")
	}
	fmt.Println("expXMLconvert: ALL DONE ==================================")
	return nil
}
